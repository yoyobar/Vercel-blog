...
![sample](/img/2024-05-20-21-53-58.png)
...
await은 async 내부에서만 사용할 수 잇다. 

await은 작업이 완료될 때 까지 대기 한 후 완료됫을 때 값을 리턴한다.
...
export function Farm() {
    ...
    return(
        <button onClick={farm} className="p-2 bg-indigo-400 hover:bg-indigo-600 transition text-white rounded-md">호출 테스트</button>
    )
}
...
이렇게 작성 할 경우 문제없이 에러의 대한 핸들링이 가능하며, 가독성 좋은 코드를 완성했다.

일반적으로 `promise`문법의 경우 `async`, `awai`t과 `try`,`catch`를 같이 사용한다고 한다.

그래도 프로미스의 기본구조를 모르면 이해하기 힘드므로 꼭 기억하자.