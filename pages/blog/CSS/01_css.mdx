...
<br/>
## CSS회고
>CSS (Cascading Style Sheets)는 사용자에게 문서를 표시하는 방법을 지정하는 <br/> Style sheet 언어입니다.
...
우선 `요소`의 정의를 알아보자. 계속해서 등장하는 `요소`라는 단어는<br/>
`<div > </div>` , 즉 태그의 묶음 자체를 `요소(Element)` 라고 칭한다.<br/>
이런 요소의 `display` 방식에는 여러가지가 존재한다.
...
약간 애매한 역할이 `inline-block` 인데, <br/>
표기 자체는 `inline` 요소로 표기하지만, `block`의 성질을 따라 조절되는 요소다.
...
예시로 `<p>` 라는 `inline` 요소에 `inline-block`을 할당해뒀다. <br/>
원래 `<p>` 라는 `inline` 요소에는 길이 관련된 속성은 붙을 수 없다. <br/><br/>

<Preview>
<p className="inline-block w-[50px] h-[50px] bg-green-600">생활</p>
</Preview>
하지만 실제로는 적용된 것을 확인할 수 있다, 이것이 `inline-block ` 이다.
...
```
<Preview>
![sample](/img/2024-05-20-17-10-01.png)
</Preview>

두 개의 `img` 를 `float`을 통해 왼쪽으로 나열하고, 아래쪽에 `<p>` 를 배치한 상태다.


모든 항목이 서로 붙어있는 걸 확인할 수 있다. 뭐가 달라졌는지 모르겠는가?<br/>
`style`속성을 제외하면 어떻게 되는지 확인해보자.

<Preview>
![sample](/img/2024-05-20-17-15-53.png)
</Preview>
`<img>`끼리는 `block`요소에 공간이 남아 붙어있지만,<br/>
`<p>`요소는 배치 순서에 따라 아래로 들어가게 된다.<br/>
그렇다면, `float`을 서로 좌, 우로 배치하게 된다면 어떻게 될까?

<Preview>
![sample](/img/2024-05-20-17-17-10.png)
</Preview>
`<img>`태그 중간에 `<p>`요소가 껴있는 다소 보기 좋지않은 상황이 벌어져 있다.<br/>
자, 그러면 우리는 `inline` 요소인 `img` 만 좌,우로 배치하고 메세지는 원래 위치로 보내고 싶다.<br/>
그럴 때는 `clear` 속성을 사용하면 된다.<br/> `clear`에는 `left` `right` `both` 가 존재하니, 상황에 맞게 쓰자.
...
<Preview>
![sample](/img/2024-05-20-17-17-45.png)
</Preview>
...
`block` 요소를 조절할 때, `display:flex` 를 이용하는게 일반적이나<br/>
그렇지 않은 경우 각 물체의 위치를 어떻게 지정하면 되는지 알아보자.<br/>
`position` 은 block의 위치를 조절하는데 쓰이는 속성이다.

| Type | Description |
| --- | --- |
| `relative` | block의 현재 위치를 기준으로 이동한다. |
| `absolute` | block의 절대 위치를 기준으로 이동한다. <br/>상위 요소에 위치 값이 존재할 경우 그 위치를 기준으로 이동한다. |
| `fixed` | block의 화면 위치를 기준으로 이동한다. <br/>화면을 따라다니는 효과가 있어, `nav` 등 다양하게 사용된다. |
...
공학을 전공하거나 좌표를 배워봤다면 Z축이라는 존재를 알고 있을 것이다.<br/>
Z축과 사용 방법은 같다.

요소를 다루다 보면 서로 겹치거나 하는 경우가 있을텐데<br/>
더 위에 나타나게 하거나 아래로 보낼 수 있는것이다.

> `z-index` 는 `position=”static”` 을 제외한 값만 변동시킬 수 있다.


위의 내용을 반드시 기억하자. 기본적으로 `position`은 `static` 인데, <br/>이러한 `position`을 부여 하지 않고 `z-index` 를 사용한다면 아무런 변화가 없을 것이다.
...
그동안 요소를 선택 할 때, `.class` `#id` 와 같은 간단한 선택자만 사용 했을 것이다.<br/>
하지만 현업에서는 대부분 모듈화를 통해 원하는 부분을 떼어서 css를 설계한다.<br/>
...
<Callout>
근데, 표를 보니까 `공백` 과 `>` 는 뭐가 다른거지?
</Callout> 

하위 선택자는 자식 선택자에 비해 더 넓은 범위를 가진다.
 
다르게 말하자면,
`div > p` 의 경우`div`의 자식 요소인 `p`에만 해당되는 속성이란 것이다.<br/>
...
요소가 어떤 상태가 되었을 때 요소를 선택하겠다는 의미다.<br/>
물론 전부 그렇게 작동하진 않는다.
...
<Callout type="info">
`nth-child`의 정수 부분에는 `n`과 같은 정수도 들어간다. <br/>홀수/짝수 등 다양하게 응용 가능하고 사용할 곳이 많은 클래스다.
</Callout>


| pseudo element | 예시, 사용 방법 |
| --- | --- |
| ::after | div::after div 후에 적용할 구성을 넣는다. 
이미 완성된 서식에 추가할 때 용이하다. |
...
의사요소와 클래스도 전부 외우자! 하지 말고 필요할 때 마다 찾아보며 익숙해지자.<br/>
...
<Preview>
<div className="bg-slate-200 p-2 rounded-md hover:bg-slate-700 hover:text-white duration-500 delay-100 transition linear">
Test
</div>
</Preview>
...
| direction | alternate  정 방향 재생 후 역방향 재생<br/>normal 정 방향 재생<br/>reverse 역 방향 재생 |
...
<Preview>
<div className="w-[50px] h-[50px] bg-slate-200 animate-spin delay-200">Box</div>
</Preview>

해석하자면 `rotate:0deg`에서 시작해 `rotate:360deg`로 움직이고 다시 돌아가는 요소이다.<br/>
...
프론트엔드를 공부한다는 것은 미디어 쿼리를 잘 다뤄야 한다.<br/>
아무리 `viewport` 를 설정한다 하더라도 <br/>실제 모바일 웹에서 보는 화면과 PC로 보는 화면은 다를 수 밖에 없다. 그럴때 사용하는 모듈이 `@media` 다. 

현재 협업에서 CSS는 각각 모듈화 해서 상황에 맞게 조립해서 사용한다는 말은 들어 봤을 것이다. <br/>그렇게 모듈화한 데이터를 `@media screen`에 맞게 각각 조절해서 사용자에게 서비스를 제공하는 것이다.
